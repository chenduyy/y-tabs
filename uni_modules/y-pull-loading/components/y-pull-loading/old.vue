...
<script>
	 ...
	 * @property {Number | String}    head-height	      顶部内容高度，默认单位为px，支持rpx 50
	...
	export default {
		name: "y-pull-loading",
		mixins: [TouchMixin],
		emits: ['input', 'update:modelValue', 'refresh'],
		options: {
		...
		computed: {
			...
			headStyle() {
				return {
					// height: addUnit(this.headHeight),
				}
			},
			dependStyle() {
				return {}
		...
		methods: {
			...
			onTouchEnd() {
				if (!this.untouchable && this.isBottom && this.deltaY) {
					...
					if (this.status === 'loosing') {
						this.setStatus(this.toPxHeadHeight, true);
			...
			ease(distance) {
				...
				const pullDistance = Number(this.pullDistance || this.toPxHeadHeight);
			...
			setStatus(distance, isLoading) {
				...
				if (isLoading) {
					...
					status = distance < Number(this.pullDistance || this.toPxHeadHeight) ? 'pulling' : 'loosing';
				}
